...
const TransactionTable = () => {
  ...
  const renderCellContent = (column, row) => {
    ...
    switch (column.key) {
        ...
        return (
          <span
            className={`font-medium ${
              column.key === "withdrawn" && value
                ? "debit-text"
                : column.key === "deposit" && value
                ? "credit-text"
                : "table-cell"
            }`}
          >
      ...
      case "type":
        const transactionType =
          getTransactionTypesMap().get(row.id) || "direct";
        return (
          <span
            className={`
    ...
    `}
      ...
      case "tags":
        return <div className="flex flex-wrap gap-1">{formatTags(value)}</div>;
      case "remarks":
        return <ScrollableCell content={value} maxHeight="80px" />;
      case "narration":
        return <ScrollableCell content={value} maxHeight="80px" />;
      case "settlements":
        const settlementNote = getSettlementNotesMap().get(row.id);
        return <span className="text-sm">{settlementNote || "-"}</span>;
      default:
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden transaction-table rounded-2xl">
      <div ref={tableRef} className="flex-1 overflow-auto rounded-2xl min-h-0">
        <table
          className="w-full border-collapse rounded-2xl"
